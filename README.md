...
- Creates the always-on **BaseStation** WiFi AP
...
1. On your phone or laptop, connect to the **BaseStation** WiFi network (open, no password)
...
1. Open `http://mowerbase.local` (or `http://10.42.0.1` via BaseStation AP)
...
| `/network` | BaseStation AP status, home WiFi connection |
...
If you have access to an external NTRIP caster (e.g. AUSCORS, RTK2Go), BaseStation can relay its corrections instead of using its own GPS Survey-In:
...
**Steps:**
1. Connect to the **BaseStation** WiFi network (open, no password) from your phone
...
- Or connect to the **BaseStation** AP and go to `http://10.42.0.1`
...
The Pi runs a fallback WiFi AP (**BaseStation**, open, no password) via `ap_manager.sh`. When connected to home WiFi the AP is inactive; it starts automatically within 10 seconds if home WiFi drops. The BCM43430 chip on the Pi Zero W v1 cannot run AP and home WiFi simultaneously on the same wlan0 interface.
